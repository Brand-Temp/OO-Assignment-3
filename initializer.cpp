...
PoolGame *Initializer::createPoolgame(const std::string &configFilePath, Dialog *parent, Dialog *stage3parent)
{
    ...
    if(config["stage2"].toBool())
    {
        if(config["stage3"].toBool()) {
            factory = new Stage2Factory(stage3parent);
        } else {
